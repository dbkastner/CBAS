...
function getAllMatrixRows(reOrderWv)
	...
	variable /g totalComp
	
	wavestats /q/rmd=[][1] reOrderWv
	variable mx=v_max
	for(i=0;i<=mx;i+=1)
		make /o/n=(dimsize(reOrderWv,0)) hold=selectnumber(reOrderWv[p][1]==i,NaN,reOrderWv[p][0])
		wavetransform zapNaNs,hold
		duplicate /o hold $"grp"+num2str(i)
	endfor
	if(mx==1)
		wave grp1Ind=grp0
		wave grp2Ind=grp1
	...
	if(mx==1)
		getMatrixRow(seqCntInfo,grp1Ind,grp2Ind)
	elseif(mx==3)
		getMatrixRow4(seqCntInfo,grp1Ind,grp3Ind,grp1Ind,grp2Ind,grp3Ind,grp4Ind,grp2Ind,grp4Ind,grp1Ind,grp4Ind,grp3Ind,grp2Ind)
	endif
	wave oneRow,rowInd
	...
	variable tot
	if(mx==1)
		make /o/n=(numpnts(grp1Ind)+numpnts(grp2Ind)) allGrpInd
		allGrpInd[0,numpnts(grp1Ind)-1]=grp1Ind[p]
		allGrpInd[numpnts(grp1Ind),numpnts(allGrpInd)-1]=grp2Ind[p-numpnts(grp1Ind)]
		make /o/n=(numpnts(grp1Ind)) grp1Boot
		make /o/n=(numpnts(grp2Ind)) grp2Boot
		tot=numpnts(allGrpInd)
		for(i=0;i<ResampleNumber;i+=1)
			setRandomSeed /BETR (i+1)/ResampleNumber
			grp1Boot=allGrpInd[limit(floor(abs(enoise(tot))),0,tot-1)]
			grp2Boot=allGrpInd[limit(floor(abs(enoise(tot))),0,tot-1)]
			getMatrixRow(seqCntInfo,grp1Boot,grp2Boot)
	...
	elseif(mx==3)
		make /o/n=(6,2) compInfo
		compInfo[0][0]= {0,0,2,1,0,2}
		compInfo[0][1]= {2,1,3,3,3,1}
		for(i=0;i<dimsize(compInfo,0);i+=1)
			wave wv1=$"grp"+num2str(compInfo[i][0])
			wave wv2=$"grp"+num2str(compInfo[i][1])
			make /o/n=(numpnts(wv1)+numpnts(wv2)) allGrpInd
			allGrpInd[0,numpnts(wv1)-1]=wv1[p]
			allGrpInd[numpnts(wv1),numpnts(allGrpInd)-1]=wv2[p-numpnts(wv1)]
			duplicate /o allGrpInd $"allGrpInd"+num2str(compInfo[i][0])+num2str(compInfo[i][1])
			make /o/n=(numpnts(wv1)) $"grp"+num2str(compInfo[i][0])+"boot"+num2str(compInfo[i][0])+num2str(compInfo[i][1])
			make /o/n=(numpnts(wv2)) $"grp"+num2str(compInfo[i][1])+"boot"+num2str(compInfo[i][0])+num2str(compInfo[i][1])
		endfor
		
		for(i=0;i<ResampleNumber;i+=1)
			for(j=0;j<dimsize(compInfo,0);j+=1)
				setRandomSeed /BETR (i+1)/ResampleNumber
				wave wv=$"allGrpInd"+num2str(compInfo[j][0])+num2str(compInfo[j][1])
				tot=numpnts(wv)
				wave wv1=$"grp"+num2str(compInfo[j][0])+"boot"+num2str(compInfo[j][0])+num2str(compInfo[j][1])
				wave wv2=$"grp"+num2str(compInfo[j][1])+"boot"+num2str(compInfo[j][0])+num2str(compInfo[j][1])
				wv1=wv[limit(floor(abs(enoise(tot))),0,tot-1)]
				wv2=wv[limit(floor(abs(enoise(tot))),0,tot-1)]
			endfor
			
			wave wv1=$"grp"+num2str(compInfo[0][0])+"boot"+num2str(compInfo[0][0])+num2str(compInfo[0][1])
			wave wv2=$"grp"+num2str(compInfo[0][1])+"boot"+num2str(compInfo[0][0])+num2str(compInfo[0][1])
			
			wave wv3=$"grp"+num2str(compInfo[1][0])+"boot"+num2str(compInfo[1][0])+num2str(compInfo[1][1])
			wave wv4=$"grp"+num2str(compInfo[1][1])+"boot"+num2str(compInfo[1][0])+num2str(compInfo[1][1])
			
			wave wv5=$"grp"+num2str(compInfo[2][0])+"boot"+num2str(compInfo[2][0])+num2str(compInfo[2][1])
			wave wv6=$"grp"+num2str(compInfo[2][1])+"boot"+num2str(compInfo[2][0])+num2str(compInfo[2][1])
			
			wave wv7=$"grp"+num2str(compInfo[3][0])+"boot"+num2str(compInfo[3][0])+num2str(compInfo[3][1])
			wave wv8=$"grp"+num2str(compInfo[3][1])+"boot"+num2str(compInfo[3][0])+num2str(compInfo[3][1])
			
			wave wv9=$"grp"+num2str(compInfo[4][0])+"boot"+num2str(compInfo[4][0])+num2str(compInfo[4][1])
			wave wv10=$"grp"+num2str(compInfo[4][1])+"boot"+num2str(compInfo[4][0])+num2str(compInfo[4][1])
			
			wave wv11=$"grp"+num2str(compInfo[5][0])+"boot"+num2str(compInfo[5][0])+num2str(compInfo[5][1])
			wave wv12=$"grp"+num2str(compInfo[5][1])+"boot"+num2str(compInfo[5][0])+num2str(compInfo[5][1])
			
			getMatrixRow4(seqCntInfo,wv1,wv2,wv3,wv4,wv5,wv6,wv7,wv8,wv9,wv10,wv11,wv12)
			duplicate /o oneRow $"root:resampDF:row"+num2istr(i)
			duplicate /o rowInd $"root:resampDF:rowInd"+num2istr(i)
	...
	killwaves /z oneRow,rowInd,grp1Boot,grp2Boot,grp0,grp1,allGrpInd,grp3Boot,grp4Boot,grp2,grp3,grp3Boot,grp4Boot
			
	return 1
end

//assumes fewer than 2^32 comparisons
Function getMatrixRow(seqSizeWv,grp1Ind,grp2Ind)
	wave seqSizeWv,grp1Ind,grp2Ind
	...
	for(j=0;j<dimsize(seqSizeWv,0);j+=1)
		...
		for(seqNum=0; seqNum<numSeq;)
			for(i=0; i<nthreads; i+=1)
				ThreadStart threadGroupID,i,getSingleMatrixEntry(cntWv,grp1Ind,grp2Ind,oneRow,seqNum,cnt+seqNum)
				seqNum+=1
				if( seqNum>=numSeq )
					break
				endif
...
//assumes fewer than 2^32 comparisons
Function getMatrixRow4(seqSizeWv,wv1,wv2,wv3,wv4,wv5,wv6,wv7,wv8,wv9,wv10,wv11,wv12)
	wave seqSizeWv,wv1,wv2,wv3,wv4,wv5,wv6,wv7,wv8,wv9,wv10,wv11,wv12
	...
	for(j=0;j<dimsize(seqSizeWv,0);j+=1)
		...
		for(seqNum=0; seqNum<numSeq;)
			for(i=0; i<nthreads; i+=1)
				ThreadStart threadGroupID,i,getSingleMatrixEntry4(cntWv,wv1,wv2,wv3,wv4,wv5,wv6,wv7,wv8,wv9,wv10,wv11,wv12,oneRow,seqNum,cnt+seqNum)
				seqNum+=1
				if( seqNum>=numSeq )
					break
				endif
...
ThreadSafe Function getSingleMatrixEntry(wv,grp1Ind,grp2Ind,outWv,which,cnt)
	WAVE wv,grp1Ind,grp2Ind,outWv
	Variable which,cnt
	
	get2waveCompStat(wv,grp1Ind,grp2Ind,outWv,which,cnt,2,0)
	
	return stopMSTimer(-2)		// time when we finished
End

ThreadSafe Function getSingleMatrixEntry4(wv,wv1,wv2,wv3,wv4,wv5,wv6,wv7,wv8,wv9,wv10,wv11,wv12,outWv,which,cnt)
	WAVE wv,wv1,wv2,wv3,wv4,wv5,wv6,wv7,wv8,wv9,wv10,wv11,wv12,outWv
	Variable which,cnt
	
	get2waveCompStat(wv,wv1,wv2,outWv,which,cnt,12,0)
	get2waveCompStat(wv,wv3,wv4,outWv,which,cnt,12,2)
	get2waveCompStat(wv,wv5,wv6,outWv,which,cnt,12,4)
	get2waveCompStat(wv,wv7,wv8,outWv,which,cnt,12,6)
	get2waveCompStat(wv,wv9,wv10,outWv,which,cnt,12,8)
	get2waveCompStat(wv,wv11,wv12,outWv,which,cnt,12,10)
	
	return stopMSTimer(-2)		// time when we finished
End

threadsafe function get2waveCompStat(wv,wv1,wv2,outWv,which,cnt,mult,add)
	wave wv,wv1,wv2,outWv
	variable which,cnt,mult,add
	
	make /o/n=(numpnts(wv1)) set=wv[wv1[p]][which]
	wavestats /q set
	variable top=v_avg
	variable bot=v_sem^2
	
	make /o/n=(numpnts(wv2)) set=wv[wv2[p]][which]
	wavestats /q set
	top-=v_avg
	bot+=v_sem^2
	bot=sqrt(bot)
	if(top==0 || (abs(bot)>=0)==0 || bot==0 || (abs(bot)>=0)==0)
		outWv[cnt*mult+add]=Nan
		outWv[cnt*mult+add+1]=NaN
	else
		if(top>0)
			outWv[cnt*mult+add]=top/bot
			outWv[cnt*mult+add+1]=NaN
		else
			outWv[cnt*mult+add]=NaN
			outWv[cnt*mult+add+1]=-top/bot
		endif
	endif
end

function getEveryCnt(reOrderWv)
	wave reOrderWv
	wave seqCntInfo
	...
	wavestats /q/rmd=[][2] seqCntInfo
	make /o/n=(v_sum,mx+3) everyCnt=NaN
	...
	for(j=0;j<dimsize(seqCntInfo,0);j+=1)
		...
		everyCnt[cnt,cnt+numSeq-1][0,1]=seqCntInfo[j][q]
		everyCnt[cnt,cnt+numSeq-1][2]=p-cnt
		for(seqNum=0; seqNum<numSeq;)
			for(i=0; i<nthreads; i+=1)
				ThreadStart threadGroupID,i,getCnts(cntWv,reOrderWv,everyCnt,mx,seqNum,cnt+seqNum)
				seqNum+=1
				if( seqNum>=numSeq )
					break
				endif
	...
	duplicate /o everyCnt $"everyCnt_"+nameOfWave(reOrderWv)
end

ThreadSafe Function getCnts(cntWv,rWv,cWv,mx,which,cnt)
	WAVE cntWv,rWv,cWv
	Variable mx,which,cnt
	
	variable i
	for(i=0;i<mx;i+=1)
		make /o/n=(dimsize(rWv,0)) pnts=selectnumber(rWv[p][1]==i,NaN,rWv[p][0])
		wavetransform zapNaNs,pnts
		if(numpnts(pnts)>0)
